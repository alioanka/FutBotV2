...
class BinanceClient:
    ...
    async def create_order(self, symbol, side, quantity, order_type="MARKET", **kwargs):
        """Robust order creation with enhanced verification"""
        params = {
            'symbol': symbol,
            'side': side,
            'type': order_type,
            ...
            'newOrderRespType': 'FULL',  # Get complete execution details
        ...
        try:
            ...
            # Enhanced fill verification
            if order_type == "MARKET":
                if not response.get('fills'):
                    # Wait and verify with separate API call
                    await asyncio.sleep(1.5)  # Increased delay for testnet
                    verified_order = await self.get_order(symbol, response['orderId'])
                    
                    if not verified_order.get('fills'):
                        # Final verification through position check
                        positions = await self.get_position_risk()
                        position = next((p for p in positions if p['symbol'] == symbol), None)
                        
                        if not position or float(position['positionAmt']) == 0:
                            raise Exception(f"No fill confirmed. Order status: {verified_order.get('status')}")
                        
                        # Reconstruct fill information from position
                        response['fills'] = [{
                        ...
                        response['executedQty'] = position['positionAmt']
                        response['avgPrice'] = position['entryPrice']
