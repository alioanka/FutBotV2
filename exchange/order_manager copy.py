...
class OrderManager:
    ...
    async def place_order(self, signal):
        """Complete order placement with all safeguards"""
        symbol = signal["symbol"]
        
        try:
            # 1. Check and close any existing position
            ...
            # 4. Get accurate fill price
            avg_price = await self._calculate_avg_fill_price(order)
            if not avg_price:
                raise Exception("Could not determine fill price")
    ...
    async def _place_sl_tp_orders(self, symbol, side, quantity, entry_price, sl_price, take_profits):
        """Place SL/TP orders with price validation"""
        # Get current market price
        klines = await self.client.get_klines(symbol, "1m", limit=1)
        current_price = float(klines['close'].iloc[-1])
        
        # Validate stop loss price
        if (side == "BUY" and sl_price >= current_price) or \
        (side == "SELL" and sl_price <= current_price):
            new_sl_price = current_price * 0.995 if side == "BUY" else current_price * 1.005
            self.logger.warning(f"Adjusting SL from {sl_price} to {new_sl_price:.2f}")
            sl_price = new_sl_price

        # Validate take profit prices
        adjusted_tps = []
        for tp in take_profits:
            if (side == "BUY" and tp['price'] <= current_price) or \
            (side == "SELL" and tp['price'] >= current_price):
                new_tp_price = current_price * 1.005 if side == "BUY" else current_price * 0.995
                self.logger.warning(f"Adjusting TP from {tp['price']} to {new_tp_price:.2f}")
                adjusted_tps.append({'price': new_tp_price, 'percentage': tp['percentage']})
            else:
        ...
        try:
            ...
            self.logger.error(f"Failed to place SL: {str(e)}")
            raise Exception(f"Stop loss placement failed: {str(e)}")

        # Place Take Profits
        for tp in adjusted_tps:
            try:
                await self.client.create_order(
                    symbol=symbol,
                    side="SELL" if side == "BUY" else "BUY",
                ...
                self.logger.error(f"Failed to place TP: {str(e)}")
                raise Exception(f"Take profit placement failed: {str(e)}")
        ...
        return len(open_orders) >= len(adjusted_tps) + 1  # SL + TPs
